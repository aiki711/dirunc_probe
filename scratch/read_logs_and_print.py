# scratch/read_logs_and_print.py
import json
...
print("# 2 Omission Types × 4 Configurations Comparison Summary\n")

for o_type in omission_types:
    print(f"## Omission Type: {o_type.upper()}")
    print("| Layer | Configuration | Best Epoch | Macro F1 | Strict Pair Acc |")
    print("| :---: | :--- | :---: | :---: | :---: |")
    for L in layers:
        for name, folder in configs.items():
            log_path = runs_dir / folder / f'{o_type}_layer_{L}' / 'log.jsonl'
            best = get_best_performance(log_path)
            if best:
                print(f"| {L:2d} | {name:26s} | {best['epoch']:2d} | {best['macro_f1']:.4f} | {best['pair_accuracy_strict']:.4f} |")
            else:
                print(f"| {L:2d} | {name:26s} | N/A | N/A | N/A |")
    print()
