...
def main():
    ...
    parser.add_argument("--sub_size", type=int, default=2000, help="Subset size for CV and Fusion to speed up")
    ...
    # Draw balanced subset of training data for fast CV / fast Fusion training
    np.random.seed(42)
    ...
    # Out-of-fold probability containers for training stacking (only on subset)
    P_train_base_sub = np.zeros((2 * len(sub_indices), 7))
    
    for d in range(7):
        # 1. Fit overall probe on ALL train data
        X_f = train_f_hs[:, d, :]
        X_m = train_m_hs[:, d, :]
        ...
        # 2. Get OOF probabilities ONLY on subset indices to save CPU time
        # subset has 2 parts: filled sub_indices, and missing sub_indices
    ...
    # -----------------------------------------------------------------------
    print("\n[C] Training Feature Concatenation (Subsampled for Speed)...")
    
    # Prepare fusion representations ONLY on subset to save RAM and CPU training time
    ...
    # -----------------------------------------------------------------------
    print("\n" + "="*70)
    print(" CROSS-SLOT FEATURE FUSION EVALUATION RESULTS (FAST) ")
    print("="*70)
