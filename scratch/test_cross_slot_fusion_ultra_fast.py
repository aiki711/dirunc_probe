...
def main():
    ...
    parser.add_argument("--sub_size", type=int, default=3000, help="Subset size for CV and Fusion")
    ...
    # Subsample balanced training data subset for rapid CV & Fusion fitting
    np.random.seed(42)
    sub_indices = np.random.choice(N_train, size=min(args.sub_size, N_train), replace=False)
    print(f"Subsampled {len(sub_indices)} items from training data for CV & Fusion.")
    
    # -----------------------------------------------------------------------
    # Setup A: Baseline (L2 Logistic Regression on raw space for speed and consistency)
    # -----------------------------------------------------------------------
    print("\n[A] Training Baseline (L2 Solver)...")
    probes_base = []
    P_train_base_sub = np.zeros((2 * len(sub_indices), 7))
    ...
    for d in range(7):
        ...
        # Fit main probe with L2 penalty, lbfgs (extremely fast convergence)
        clf = LogisticRegression(penalty='l2', solver='lbfgs', C=args.C, tol=1e-3, max_iter=500, n_jobs=-1, random_state=42)
        clf.fit(X_train_np, y_train_np)
        probes_base.append(clf)
        
        # Fast 5-fold CV on the subsampled subset
        X_sub_f = train_f_hs[sub_indices, d, :]
        X_sub_m = train_m_hs[sub_indices, d, :]
        X_sub = np.concatenate([X_sub_f, X_sub_m], axis=0)
        
        y_sub_f = np.zeros(len(sub_indices))
        y_sub_m = train_y_labels[sub_indices, d]
        y_sub = np.concatenate([y_sub_f, y_sub_m], axis=0)
        
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        for train_idx, val_idx in kf.split(X_sub):
            clf_cv = LogisticRegression(penalty='l2', solver='lbfgs', C=args.C, tol=1e-3, max_iter=500, n_jobs=-1, random_state=42)
            clf_cv.fit(X_sub[train_idx], y_sub[train_idx])
            P_train_base_sub[val_idx, d] = clf_cv.predict_proba(X_sub[val_idx])[:, 1]
    ...
    for d in range(7):
        ...
        clf_meta.fit(P_train_base_sub, y_train_sub_d)
        meta_classifiers.append(clf_meta)
    ...
    # -----------------------------------------------------------------------
    print("\n[C] Training Feature Concatenation (L2, Subsampled)...")
    
    X_sub_fusion_f = train_f_hs[sub_indices].reshape(len(sub_indices), 7 * 2304)
    X_sub_fusion_m = train_m_hs[sub_indices].reshape(len(sub_indices), 7 * 2304)
    X_sub_fusion = np.concatenate([X_sub_fusion_f, X_sub_fusion_m], axis=0) # [2 * N_sub, 16128]
    ...
    for d in range(7):
        ...
        # Use L2-regularized Logistic Regression (extremely fast in 16128 dimensions)
        clf_fusion = LogisticRegression(penalty='l2', solver='lbfgs', C=args.C_fusion, tol=1e-3, max_iter=500, n_jobs=-1, random_state=42)
        clf_fusion.fit(X_sub_fusion, y_train_sub_d)
        probes_fusion.append(clf_fusion)
    ...
    # -----------------------------------------------------------------------
    print("\n" + "="*70)
    print(" CROSS-SLOT FEATURE FUSION EVALUATION RESULTS (ULTRA FAST - L2) ")
    print("="*70)
