#!/usr/bin/env python3
"""
41_plot_3x2_comparison.py
===========================
3種類のデータセット（機械的削除、Soft Omission、Strong Omission）× 2種類の評価指標（Macro F1, Strict Pair Accuracy）の比較グラフを生成するスクリプト。
...
レイアウト: 3行(Mechanical/Soft/Strong) × 2列(Macro F1 / Strict Pair Acc)
"""
...
OMISSION_TYPES = ["mechanical", "soft", "strong"]

# ---------------------------------------------------------------------------
# Data loading helpers
# ---------------------------------------------------------------------------
LAYER_RE = re.compile(r"^(?:mechanical|soft|strong)_layer_(\d+)$")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate 3×2 subplot comparison figure for 4 probing configurations."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out_path", type=str, default="runs/3x2_comparison.png",
        help="出力画像パス"
    )
    parser.add_argument(
    ...
    fig.subplots_adjust(top=0.91, bottom=0.10, hspace=0.35, wspace=0.25)

    omission_labels = {
        "mechanical": "Mechanical Deletion",
        "soft": "Soft Omission",
    ...
    for row_i, omission in enumerate(OMISSION_TYPES):
        for col_j, metric in enumerate(METRICS):
            ...
            if col_j == 0:
                ...
                ax.set_ylabel(metric["display"], fontsize=9.5)

            ax.set_xlabel("Layer Index", fontsize=9)

            # Column title on top row
            if row_i == 0:
                ax.set_title(metric["display"], fontsize=11, fontweight="bold", pad=6)
    ...
    fig.suptitle(
        "Probing Performance Comparison across 3 Omission Types\n"
    ...
    md_path = out_path.with_name("comparison_summary_3x2.md")
    
    with md_path.open("w", encoding="utf-8") as f_md:
        f_md.write("# 3 Omission Types × 4 Configurations Comparison Summary\n\n")
        f_md.write("Shows Macro F1 and Strict Pair Accuracy across multiple layers for Mechanical Deletion, Soft Omission, and Strong Omission.\n\n")
