...
def collate_dual_nq_batch(tokenizer, batch: List[Dict[str, Any]], max_length: int) -> Dict[str, Any]:
    texts_real = [x["text"] + NATURAL_QUERY_STR for x in batch]
    texts_null = [NATURAL_QUERY_STR.strip()] * len(batch)
...
def main():
    ...
    print(f"=======================================")
    print(f"Experiment 6f: Natural Query LODO Evaluation (Domain: {args.test_domain})")
    print(f"=======================================\n", flush=True)
    ...
    def get_predictions(rows: List[Dict[str, Any]], probe: NaturalQueryProbe) -> Tuple[np.ndarray, np.ndarray]:
        ds = EvaluatorDataset(rows)
        dl = DataLoader(ds, batch_size=args.batch_size, shuffle=False, 
                        collate_fn=lambda b: collate_dual_nq_batch(tokenizer, b, max_length=1024))
        y_true, delta_p_list = [], []
        probe.eval()
        with torch.no_grad():
    ...
    base.lm.eval()
    probe = NaturalQueryProbe(base, tokenizer, args.model_path).to(device)
