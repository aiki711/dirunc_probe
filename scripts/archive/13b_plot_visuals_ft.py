...
def plot_trajectory_line(target_id, data, out_dir):
    ...
    plt.title(f"[Final Token] Dynamic Checklist: '{data['text']}'", fontsize=13)
...
def plot_heatmap(target_id, data, out_dir):
    ...
    plt.title(f"[Final Token] Checklist State Matrix: '{data['text']}'", fontsize=13, pad=20)
    plt.tight_layout()
...
def plot_radar(target_id, data, out_dir):
    ...
    ax.set_ylim(0, 100)
    plt.title("[Final Token] Checklist Morphing (Radar)", size=16, y=1.1)
...
def main():
    out_dir = Path("runs/balanced/experiment9_visuals_ft")
    data_file = out_dir / "trajectory_data.json"
    
    if not data_file.exists():
        print(f"Data file not found: {data_file}. Run 13a_extract_trajectory_ft.py first.")
        return
    ...
    print(f"All visualizations (Final Token) saved to {out_dir}")
