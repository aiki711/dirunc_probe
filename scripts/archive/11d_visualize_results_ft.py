...
def main():
    results_path = "runs/balanced/experiment7_neurons_ft/activation_shifts.json"
    if not Path(results_path).exists():
        print("Results file not found at:", results_path)
        return
    ...
    for item in data:
        ...
        print(f"\n--- [{label.upper()}] Minimal Pair ---")
        print(f"Missing (A): {item.get('A_text', 'N/A')}")
        print(f"Filled  (B): {item.get('B_text', 'N/A')}")
        
        # We'll just show the top 3 neurons for brevity in ASCII
        for i in range(min(5, len(top_neurons))):
            idx = top_neurons[i]
            act_a = item.get("A_activations", [])[i] if "A_activations" in item else 0
            act_b = item.get("B_activations", [])[i] if "B_activations" in item else 0
            shift = act_b - act_a
            
            row_label = f"{label} n{idx}" if i == 0 else f"      n{idx}"
            
            # Line for A (Missing)
            p_a = f"{item.get('A_prob', 0):.2%}" if i == 0 else ""
            print(f"{row_label:<10} | {'Missing':<8} | {p_a:<8} | {draw_bar(act_a)}")
            
            # Line for B (Filled)
            p_b = f"{item.get('B_prob', 0):.2%}" if i == 0 else ""
