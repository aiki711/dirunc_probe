...
# --- Configuration ---
SOURCE_PATH = Path("data/processed/case_grammar/natural_dev.jsonl")
OUTPUT_PATH = Path("data/processed/case_grammar/natural_dev_gemini.jsonl")

# Refined models based on success and availability
MODELS = [
    "gemini-2.0-flash",           # High performance
    "gemini-flash-latest",        # Stable 1.5
    "gemini-pro-latest",          # High quality
    "gemini-2.0-flash-lite-001"   # Lite/Fast
]
        ...
        with OUTPUT_PATH.open("a", encoding="utf-8") as out_f:
            for res in results:
                if isinstance(res, dict):
                    ...
                    saved_count += 1
            out_f.flush()
        print(f"Finished chunk {i//chunk_size + 1}. Saved {saved_count} samples.")

    print(f"Generation completed. Results at {OUTPUT_PATH}")
