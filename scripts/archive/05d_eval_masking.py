...
def mask_text(text: str) -> str:
    # Pattern to match the keywords case-insensitively, using word boundaries
    pattern = re.compile(r'\b(' + '|'.join(MASK_KEYWORDS) + r')\b', re.IGNORECASE)
    # the model tokenizer might not have a [MASK] token if it's an auto-regressive model like gemma,
    # so we can use a generic placeholder word like "[MASK]" or "something"
...
def main():
    ...
    # Add [MASK] to vocab if not present
    ...
    print(f"Extracting activations for layer {args.layer_idx} (MASKING)...")
    ...
    print(f"\n[Masking Overall] Macro F1: {macro_f1:.4f}")
