...
def build_diff_dataset(data):
    f_hs = data["f_hs"]          # [B, D] or [B, 7, D]
    m_hs = data["m_hs"]          # [B, D] or [B, 7, D]
    metadata = data["metadata"]
    
    # Compute difference vector: missing - filled
    d_hs = m_hs - f_hs
    
    # Flatten token dimensions if present to avoid positional shortcut
    if len(d_hs.shape) == 3:
        # Flatten [B, 7, D] to [B, 7*D]
    ...
    # Convert list of tensors to a single tensor and convert BF16 to Float32 for training
...
def main():
    ...
    print(f"Training Case Role Classifiers using Difference vectors from '{args.prefix}'...")
    ...
    for L in layers:
        ...
        train_ds = build_diff_dataset(train_data)
        dev_ds = build_diff_dataset(dev_data)
    ...
    plt.plot(layers_role, accs_role, marker='o', color="#1B5E20", linewidth=2.5, label="Difference Vector Classifier Accuracy")
    ...
    raw_results = {
        "prefix": args.prefix,
        "layers": layers,
        "diff_classification_accuracy": {str(L): acc for L, acc in role_accs},
