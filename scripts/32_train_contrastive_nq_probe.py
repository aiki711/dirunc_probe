...
class NaturalQueryProbe(nn.Module):
    """Refers to the '?' token hidden state in 'Who?', 'What?', etc. appended to the text."""
    def __init__(self, base: ProbeModelBase, tokenizer: Any):
        super().__init__()
        self.base = base
        
        self.query_token_seqs = {}
        for d, qstr in NATURAL_QUERY_MAP.items():
            # encode with a leading space to match the spacing in NATURAL_QUERY_STR
            self.query_token_seqs[d] = tokenizer.encode(" " + qstr, add_special_tokens=False)
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, layer_idx: int) -> torch.Tensor:
        ...
        for bi in range(n_samples):
            ...
            dir_vecs = []
            for d in DIRS:
                seq = self.query_token_seqs[d]
                pos = None
                # Search for the query sequence from the end
...
def main():
    ...
    parser.add_argument("--out_dir",       type=str,   default="runs/contrastive_nq_probe")
    ...
    optim = torch.optim.AdamW([model.W, model.b], lr=args.lr)
